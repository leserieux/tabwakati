...
export default async function UserDetailPage({ params }: { params: { id: string } }) {
  ...
  for (const row of balances.rows) {
    ...
    current.available += Number(row.available_balance || 0);
    current.staking += Number(row.staking_balance || 0);
    current.pending += Number(row.pending_balance || 0);
    assetBalances.set(row.asset_symbol, current);
  }

  const assets: AssetBalance[] = [...assetBalances.entries()].map(([asset, value]) => {
    const total = value.available + value.staking + value.pending;
    const priceUsd = prices.get(asset) ?? null;
    return {
      asset,
      ...
      total,
      priceUsd,
      valueUsd: priceUsd === null ? null : total * priceUsd
    };
  }).sort((a, b) => b.total - a.total);

  const totalUsd = assets.reduce((sum, row) => sum + (row.valueUsd ?? 0), 0);
  const activeAssets = assets.filter((row) => row.total > 0).length;
  ...
  return (
    <div>
      ...
      <div className="wk-panel" style={{ marginBottom: 18 }}>
        <div className="wk-asset">{user.email || "Email non renseigné"}</div>
        <div className="wk-asset-sub">ID : {user.id}</div>
        <div className="wk-asset-sub">Créé le : {user.created_at ? formatDateTime(new Date(user.created_at)) : "—"} · Dernière activité : {user.last_activity_at ? formatDateTime(new Date(user.last_activity_at)) : "—"}</div>
      </div>

      <div className="wk-strip">
        <StatCard label="Solde total" value={formatUsd(totalUsd)} sub="Valorisation USD" />
        <StatCard label="Actifs détenus" value={String(activeAssets)} sub="Catégories actives" />
        <StatCard label="Transactions" value={String(transactions.rows.length)} sub="Dernières 30 opérations" />
        <StatCard label="Échecs" value={String(failed)} sub="Opérations en échec" tone={failed > 0 ? "bad" : "ok"} />
      </div>

      <Section title="Soldes par actif" hint="Aucune quantité de différents actifs n'est additionnée.">
        {assets.length === 0 ? (
          <Empty text="Aucun solde pour cet utilisateur." />
        ) : (
          <TableWrap>
            <thead>
              <tr>
                ...
                <Th right>Total</Th>
                <Th right>Valeur USD</Th>
              </tr>
            </thead>
            <tbody>
              {assets.map((row) => (
                <tr key={row.asset}>
                  <Td label="Actif"><span className="wk-asset">{row.asset}</span></Td>
                  <Td right label="Prix">{row.priceUsd === null ? "—" : formatUsd(row.priceUsd)}</Td>
                  <Td right label="Disponible">{formatNumber(row.available)}</Td>
                  <Td right label="Staking">{formatNumber(row.staking)}</Td>
                  <Td right label="En attente">{formatNumber(row.pending)}</Td>
                  <Td right label="Total">{formatNumber(row.total)}</Td>
                  <Td right label="Valeur USD">{row.valueUsd === null ? "—" : formatUsd(row.valueUsd)}</Td>
                </tr>
              ))}
            </tbody>
          </TableWrap>
      ...
      <Section title="Transactions récentes" hint={`${transactions.rows.length} opération(s) affichée(s)${failed ? ` · ${failed} échec(s)` : ""}`}>
        {transactions.rows.length === 0 ? (
          <Empty text="Aucune transaction pour cet utilisateur." />
        ) : (
          <TableWrap>
            <thead>
              <tr>
            ...
            <tbody>
              {transactions.rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <Td label="Actif">{row.asset_symbol}</Td>
                  <Td right label="Montant">
                    {formatNumber(Number(row.amount))}
                    {row.fee ? <div className="wk-asset-sub">Frais : {formatNumber(Number(row.fee))}</div> : null}
                  </Td>
                  <Td label="Statut"><Pill tone={txStatusTone(row.status)}>{txStatusLabel(row.status)}</Pill></Td>
                  <Td hideSm label="Référence"><span className="wk-asset-sub">{shortId(row.tx_hash || row.id)}</span></Td>
                </tr>
              ))}
            </tbody>
          </TableWrap>
...
function StatCard({ label, value, sub, tone = "ok" }: { label: string; value: string; sub: string; tone?: "ok" | "bad" | "warn" | "info" }) {
  return (
